...
export default function Home() {
  ...
  return <DashboardLayout>
    <div className="mx-auto max-w-[1540px] px-5 py-8 lg:px-9 lg:py-10">
      <div className="flex flex-col justify-between gap-6 border-b border-[#aeadA8] pb-8 md:flex-row md:items-end"><div><div className="mb-3 font-mono text-[10px] uppercase tracking-[0.25em] text-[#656560]">Painel / Visão geral</div><h1 className="font-display text-4xl font-black uppercase leading-[0.92] tracking-[-0.065em] sm:text-6xl">Bom dia,<br /><span className="text-[#777772]">Oficina Norte.</span></h1></div><div className="flex gap-2"><button className="flex items-center gap-2 border border-[#90908b] bg-[#f0f0ed] px-4 py-3 font-mono text-[10px] uppercase tracking-[0.16em] hover:bg-white"><CalendarDays size={15} />01 — 31 AGO</button><button className="flex items-center gap-2 bg-[#171717] px-4 py-3 font-mono text-[10px] uppercase tracking-[0.16em] text-white hover:bg-[#30302e]"><Plus size={15} />Lançar</button></div></div>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.45fr_0.8fr]">
        <div className="border border-[#b8b8b3] bg-[#f1f1ee] p-5 shadow-[4px_4px_0_#c3c3be] sm:p-7"><div className="flex items-start justify-between"><div><div className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#696965]">Fluxo de caixa</div><div className="mt-2 font-display text-xl font-bold tracking-[-0.04em]">Movimentação acumulada</div></div><button className="border border-[#c5c5c0] p-2 text-[#5c5c57] hover:bg-white"><MoreHorizontal size={17} /></button></div><div className="mt-9 flex h-[210px] items-end gap-2 border-b border-l border-[#c6c6c1] px-3 pb-0 sm:gap-4 sm:px-5">{bars.map((height, index) => <div key={index} className="group relative flex h-full flex-1 items-end"><div className={`w-full ${index > 8 ? "bg-[#191919]" : "bg-[#969692]"} transition-all group-hover:bg-[#4e4e4a]`} style={{ height: `${height}%` }} /><span className="absolute -bottom-6 left-1/2 -translate-x-1/2 font-mono text-[9px] text-[#858580]">{String(index + 1).padStart(2, "0")}</span></div>)}</div><div className="mt-10 flex items-center justify-between font-mono text-[9px] uppercase tracking-[0.16em] text-[#777772]"><span>01 AGO</span><span>Período atual · 31 dias</span><span>31 AGO</span></div></div>
        <div className="relative overflow-hidden border border-[#242424] bg-[#242424] p-6 text-[#f1f1ed] shadow-[4px_4px_0_#a7a7a2]"><div className="absolute -right-12 -top-12 h-40 w-40 border-[24px] border-[#3c3c3a]" /><div className="relative"><div className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#9d9d98]">Leitura do período</div><div className="mt-12 font-display text-5xl font-black tracking-[-0.08em]">50,2<span className="text-2xl">%</span></div><p className="mt-3 max-w-[210px] text-sm leading-6 text-[#b8b8b3]">da receita permaneceu disponível depois das despesas operacionais.</p><div className="mt-10 border-t border-[#525250] pt-4"><div className="flex justify-between font-mono text-[10px] uppercase tracking-[0.12em]"><span className="text-[#9d9d98]">Eficiência</span><span>Acima da meta</span></div><div className="mt-3 h-2 bg-[#4b4b49]"><div className="h-full w-[72%] bg-[#dadad5]" /></div></div></div></div>
      </section>
